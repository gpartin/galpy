...
class LFMPotential(Potential):
    """Class that implements modified gravity using the LFM Radial Acceleration
    Relation (RAR).

    Given a baryonic potential, the effective gravitational acceleration is
    ...
    magnitude derived from the wrapped potential(s) and :math:`a_0 = c H_0 /
    (2\\pi)` is the LFM fundamental acceleration scale.

    The force direction is preserved: only the magnitude is boosted. This
    relation interpolates between Newtonian gravity
    (:math:`g_\\mathrm{bar} \\gg a_0`) and the deep-field regime
    (:math:`g_\\mathrm{bar} \\ll a_0`) where
    :math:`g_\\mathrm{obs} \\approx \\sqrt{g_\\mathrm{bar}\\,a_0}`.

    The potential energy is computed via numerical quadrature along the radial
    direction. Because the deep-field force decays as :math:`1/r`, the
    potential diverges logarithmically; a finite integration cutoff ``rmax``
    is used (default 1000 natural units).
    ...
    a0 : float or Quantity, optional
        ...
        (vo\\ :sup:`2`/ro).  If an astropy Quantity with acceleration
        dimensions, converted automatically.  Default: :math:`c H_0/(2\\pi)`
        using ``H0`` and the potential's unit system.
    H0 : float or Quantity, optional
        Hubble constant in km/s/Mpc (only used when ``a0`` is not given).
        Default: 67.4.
    rmax : float, optional
        Integration cutoff (natural units) for potential evaluation.
        Default: 1000.
    ro : float or Quantity, optional
        Distance scale for translation into internal units (default from
        configuration file).
    vo : float or Quantity, optional
        Velocity scale for translation into internal units (default from
        configuration file).

    Notes
    -----
    - 2026-03-09 - Written - Partin
    - See Partin (2026) LFM-PAPER-045 for the derivation from lattice
      field dynamics.
    - McGaugh, Lelli & Schombert (2016) PRL 117, 201101 observationally
      established the radial acceleration relation in SPARC galaxies.
